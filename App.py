...
HOP_LENGTH = 512     # Hop length for frame-based analysis
MIN_FREQUENCY = 65   # C2 - lowest note to detect
MAX_FREQUENCY = 2093 # C7 - highest note to detect
VOICING_THRESHOLD = 0.5  # Confidence threshold for pitch detection

# Voice type ranges (MIDI notes)
VOICE_TYPES = {
    'Bass': {'min': 40, 'max': 64, 'tessitura': (45, 57)},      # E2-E4
    'Baritone': {'min': 45, 'max': 69, 'tessitura': (50, 62)},  # A2-A4
    'Tenor': {'min': 48, 'max': 72, 'tessitura': (55, 67)},     # C3-C5
    'Alto': {'min': 53, 'max': 77, 'tessitura': (60, 72)},      # F3-F5
    'Mezzo-Soprano': {'min': 57, 'max': 81, 'tessitura': (64, 76)}, # A3-A5
    'Soprano': {'min': 60, 'max': 84, 'tessitura': (67, 79)}    # C4-C6
}
...
def smooth_pitch(pitch: np.ndarray, confidence: np.ndarray, 
    ...
    Args:
        ...
        confidence: Confidence/voicing probability for each frame
        window_size: Window size for median filter
        conf_threshold: Minimum confidence to consider voiced
    ...
    # Apply median filter to non-zero values
...
def detect_pitch_crepe(y: np.ndarray, sr: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Tuple of (pitch in Hz, confidence)
    """
...
def detect_pitch_yin(y: np.ndarray, sr: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        Tuple of (pitch in Hz, confidence proxy)
    """
    pitch = librosa.yin(
        y,
        fmin=MIN_FREQUENCY,
    ...
    # Compute confidence proxy from RMS energy
...
def classify_voice_type(midi_notes: np.ndarray, tessitura_range: Tuple[float, float]) -> Dict:
    ...
    Args:
        ...
        tessitura_range: Tuple of (low, high) tessitura MIDI notes
        
    Returns:
        Dictionary with voice type and confidence
    """
    ...
    is_borderline = (len(sorted_types) > 1 and 
                     sorted_types[1][1] > best_score * 0.85)
...
def analyze_audio(y: np.ndarray, sr: int, progress_callback=None) -> Dict:
    ...
    # Smooth pitch
    pitch_smoothed = smooth_pitch(pitch, confidence, window_size=5, 
                                   conf_threshold=VOICING_THRESHOLD)
    ...
    # Step 3: Convert to MIDI and note names
    if progress_callback:
    ...
    # Step 4: Voice type classification
    if progress_callback:
    ...
    if not os.path.exists(CATALOG_FILE):
        ...
        demo_songs = [
            ...
            },
            {
                ...
                'typical_low': 'C3',
                'typical_high': 'C5',
                'tags': 'ballad,emotional,mid-forward',
            ...
            },
            {
                ...
                'typical_low': 'F3',
                'typical_high': 'Ab5',
                'tags': 'classical,sacred,treble-bright',
                'difficulty': 'advanced'
            },
            {
                'title': 'O Holy Night',
                ...
                'typical_low': 'C3',
                'typical_high': 'C5',
                'tags': 'christmas,powerful,balanced',
                'difficulty': 'advanced'
            },
            {
                'title': 'Edelweiss',
                ...
                'typical_low': 'Bb3',
                'typical_high': 'Eb4',
                'tags': 'gentle,musical,mid-forward',
                'difficulty': 'beginner'
            },
            {
                'title': 'Danny Boy',
            ...
            },
            {
                ...
                'typical_low': 'E3',
                'typical_high': 'E4',
                'tags': 'folk,haunting,mid-forward',
                'difficulty': 'beginner'
            },
            {
...
def recommend_songs(analysis_results: Dict, catalog: pd.DataFrame) -> Dict:
    ...
    Args:
        analysis_results: Voice analysis results
        catalog: Song catalog DataFrame
    ...
    for _, song in catalog.iterrows():
        ...
        song_range = song_high_midi - song_low_midi
        
        # Check if song fits within tessitura (±1 semitone)
        tess_margin = 1
        fits_tessitura = (
        ...
        elif (song_high_midi <= user_max + 4 and song_low_midi >= user_min - 4 and
              range_overlap > 0):
            # Stretch song (extends 2-4 semitones)
    ...
    # Sort by difficulty and compatibility
    ...
    with tab1:
        ...
        with col2:
            ...
            if not WEBRTC_AVAILABLE:
                ...
                # WebRTC component (placeholder - full implementation would require audio callback)
                st.markdown("""
                <div class='info-card'>
        ...
        if st.session_state.analysis_results is None:
            ...
            if 'error' in results:
                ...
                with col3:
                    ...
                    score_color = '#10b981' if intonation_score >= 80 else '#f59e0b' if intonation_score >= 60 else '#ef4444'
                ...
                with col_right:
                    ...
                    <div class='info-card'>
                        ...
                        {'Your voice has rich low frequencies - great for warmth and depth!' if 'Bass' in timbre else
                         'Your voice has bright high frequencies - excellent for clarity!' if 'Treble' in timbre else
                         'Your voice has strong mid frequencies - perfect for presence!' if 'Mid' in timbre else
                         'Your voice is well-balanced across all frequencies!'}
                    </div>
                    """, unsafe_allow_html=True)
